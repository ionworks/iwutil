...
def check_and_combine_options(
    default_options, custom_options=None, filter_unknown=False
) -> dict:
    """
    ...
    if filter_unknown:
        custom_options = {
            k: v for k, v in custom_options.items() if k in default_options
        }
    else:
    ...
    for k, v in default_options.items():
        if v == "[required]" and k not in custom_options:
            ...
            if k in custom_options:
                if (
                    v.any_value_allowed is False
                    and custom_options[k] not in v.allowed_values
                ):
